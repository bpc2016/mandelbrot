// Package math contains math routines to support Mandelbrot.
package math
...
// RandPermutation is Knuth's algorithm for producing a random permuation on symbols 0 .. N-1
func RandPermutation(N int) []int {
	t := time.Now().Nanosecond()
	rand.Seed(int64(t))
	var v []int
	for i := 0; i < N; i++ { // start with the identity
		v = append(v, i)
	}
	for i := 0; i < N-1; i++ { // perform random swaps
...
// RandPermFunc returns a permutation function which can be applied to an integer, based
...
// Base256 returns an an array of the base 256 representation of v, which is presumed 24 bit.
// Our use case is an RGB color
